

import pytest
from rest_framework.test import APIClient
from model_bakery import baker
from students.models import Course, Student

url = 'http//localhost:8000/courses'

@pytest.fixture
def students_factory():
    def factory(*args, **kwargs):
        return baker.make(Student, *args, **kwargs)
    return factory

@pytest.fixture
def courses_factory():
    def factory(*args, **kwargs):
        return baker.make(Course, *args, **kwargs)
    return factory


@pytest.fixture
def client():
    return APIClient()

    
@pytest.mark.django_db
def test_get_course(client, courses_factory):
    courses = courses_factory(_quantity=1)
    
    response = client.get('/api/v1/courses/1/')
    data = response.json()
    assert response.status_code == 200
    assert data['id'] == 1
    
@pytest.mark.django_db
def test_get_courses_list(client, courses_factory):
    courses = courses_factory(_quantity = 10)
    response = client.get('/api/v1/courses/')
    data = response.json()
    assert response.status_code == 200
    for i, course in enumerate(data):
        assert course.get('name') == data[i]['name']
    
@pytest.mark.django_db    
def test_get_courses_by_id(client, courses_factory):
    courses = courses_factory(_quantity = 10)
    for course in courses:
        response = client.get(f'/api/v1/courses/?id={course.id}')
        data = response.json()
        assert response.status_code == 200
        assert data[0].get('id') == course.id

@pytest.mark.django_db
def test_get_courses_by_name(client, courses_factory):
    courses = courses_factory(_quantity = 10)
    for course in courses:
        response = client.get(f'/api/v1/courses/?name={course.name}')
        data = response.json()
        assert response.status_code == 200
        assert data[0].get('name') == course.name
        
@pytest.mark.django_db
def test_create_course(client):
    data = {'name': '123'}
    response = client.post('/api/v1/courses/', data)
    assert response.status_code == 201
    
@pytest.mark.django_db
def test_update_course(client, courses_factory):
    courses = courses_factory(_quantity = 10)
    data = {'name': '123'}
    response = client.patch(f'/api/v1/courses/{courses[0].id}/', data=data)
    assert response.status_code == 200
    assert response.data.get('name') == data['name']
    
@pytest.mark.django_db
def test_delete_course(client, courses_factory):
    courses = courses_factory(_quantity = 1)
    all_courses = len(courses)
    response = client.delete(f'/api/v1/courses/{courses[0].id}/')
    assert response.status_code == 204
    assert response.data == None
